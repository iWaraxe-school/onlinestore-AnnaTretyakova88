...
## Task #4
Starting to extend our store. Please append the ability user to interact with our store, while sending commands through read stream.
Add support of such commands: 
- `sort` - products from the store according to config. In the resources folder create xml config file like
```
xml
<sort>
    <name>asс</name>
...
```
The config file can contain from 1 to N fields. The sort should be done using `Comparator`. Sort and print should not modify
